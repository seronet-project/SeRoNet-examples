...
#include "ComponentKeyboardJoystickOpcUaBackendPortFactory.hh"
...
static ComponentKeyboardJoystickOpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentKeyboardJoystickOpcUaBackendPortFactory::ComponentKeyboardJoystickOpcUaBackendPortFactory()
{  
	componentImpl = 0;
...
void ComponentKeyboardJoystickOpcUaBackendPortFactory::initialize(ComponentKeyboardJoystick *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentKeyboardJoystickOpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentKeyboardJoystickOpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}
...
Smart::IPushServerPattern<CommBasicObjects::CommJoystick> * ComponentKeyboardJoystickOpcUaBackendPortFactory::createJoystickServiceOut(const std::string &serviceName)
{
	return new SeRoNet::OPCUA::Server::PushServer<CommBasicObjects::CommJoystick>(componentImpl, serviceName);
}


int ComponentKeyboardJoystickOpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentKeyboardJoystickOpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentKeyboardJoystickOpcUaBackendPortFactory::destroy()
{
