...
start)
	...
	xterm -fg red -title "localhost" -e 'echo;echo "Starting local session ..."; echo;echo;cd /tmp/SystemOpcUaJoystickTrafficLights.deployment; pwd; bash --login -i -e start-LocalPC.sh start' &
	echo $! >> $PID_FILE
...
stop)
	...
	xterm -title "stopping localhost" -e 'echo;echo "Starting local session ..."; echo;echo;cd /tmp/SystemOpcUaJoystickTrafficLights.deployment; pwd; bash --login -i -e start-LocalPC.sh stop'
	echo $! >> $PID_FILE
