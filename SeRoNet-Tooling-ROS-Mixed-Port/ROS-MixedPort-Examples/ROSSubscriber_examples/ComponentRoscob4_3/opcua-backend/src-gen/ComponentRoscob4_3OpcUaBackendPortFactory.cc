...
#include "ComponentRoscob4_3OpcUaBackendPortFactory.hh"
...
static ComponentRoscob4_3OpcUaBackendPortFactory OpcUaBackendPortFactory;

ComponentRoscob4_3OpcUaBackendPortFactory::ComponentRoscob4_3OpcUaBackendPortFactory()
{  
	componentImpl = 0;
	ComponentRoscob4_3::instance()->addPortFactory("OpcUa_SeRoNet", this);
}

ComponentRoscob4_3OpcUaBackendPortFactory::~ComponentRoscob4_3OpcUaBackendPortFactory()
{  }

void ComponentRoscob4_3OpcUaBackendPortFactory::initialize(ComponentRoscob4_3 *component, int argc, char* argv[])
{
	componentImpl = new SeRoNet::Utils::Component(component->connections.component.name);
}

int ComponentRoscob4_3OpcUaBackendPortFactory::onStartup()
{
	if (!component_thread.joinable()) {
    	component_thread = std::thread(&ComponentRoscob4_3OpcUaBackendPortFactory::task_execution, this);
    	return 0;
    }
	return -1;
}

Smart::IPushClientPattern<CommBasicObjects::CommJoystick> * ComponentRoscob4_3OpcUaBackendPortFactory::createJoyIn()
{
...
int ComponentRoscob4_3OpcUaBackendPortFactory::task_execution()
{
	componentImpl->run();
	return 0;
}

int ComponentRoscob4_3OpcUaBackendPortFactory::onShutdown(const std::chrono::steady_clock::duration &timeoutTime)
{
...
void ComponentRoscob4_3OpcUaBackendPortFactory::destroy()
{
