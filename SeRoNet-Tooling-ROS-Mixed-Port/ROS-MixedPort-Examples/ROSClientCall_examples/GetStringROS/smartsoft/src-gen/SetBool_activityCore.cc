...
SetBool_activityCore::SetBool_activityCore(Smart::IComponent *comp, const bool &useDefaultState) 
...
SetBool_activityCore::~SetBool_activityCore()
{
}


void SetBool_activityCore::notify_all_interaction_observers() {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	// try dynamically down-casting this class to the derived class 
	// (we can do it safely here as we exactly know the derived class)
	if(const SetBool_activity* setBool_activity = dynamic_cast<const SetBool_activity*>(this)) {
		for(auto it=interaction_observers.begin(); it!=interaction_observers.end(); it++) {
			(*it)->on_update_from(setBool_activity);
		}
	}
}

void SetBool_activityCore::attach_interaction_observer(SetBool_activityObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.push_back(observer);
}

void SetBool_activityCore::detach_interaction_observer(SetBool_activityObserverInterface *observer) {
	std::unique_lock<std::mutex> lock(interaction_observers_mutex);
	interaction_observers.remove(observer);
}

int SetBool_activityCore::execute_protected_region()
{
...
void SetBool_activityCore::updateAllCommObjects()
{
...
void SetBool_activityCore::triggerLogEntry(const int& idOffset)
{
	if(useLogging == true) {
...
int SetBool_activityCore::getPreviousCommObjId()
{
