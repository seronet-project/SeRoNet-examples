...
ROS offers components (called packages) that cover most robotics sub-domains, there are already over 3000 packages available. The ROS code, mostly hand written, lack habitually a proper formal description of its architecture, or in other words models. The ROS tooling, part of the SeRoNet tooling collection, aims to compensate this shortcoming providing a formal language that allows to the user the description of ROS basic concepts. Thanks to this language we can very easily connect ROS existing code with SeRoNet structures.

This documentation is organized as follows:

- [ROS Tooling introduction](#step-1-use-the-ros-developer-perspective-to-describe-ros-nodes)
- [SeRoNet ROS Mixed Port user documentation](#step-2-connect-the-ros-model-to-seronet-components)
- [ROS Mixed Port walk through tutorials](#ros-mixed-port-walk-through-tutorials)
...
A previous default setup of the workspace is necessary to follow the instructions of this tutorial, please check our recommendations before you continue [WORKSPACE SETUP](ROS-MixedPort-Examples/ROSMixedPortTutorials_WSsetup.md).
...
For the common ROS objects, the respective SeRoNet objects are already translated and available within this [repository](DomainRosModelsRepositories).
...
For further information please see: [SeRoNet Tooling ROS Mixed Port documentation](https://github.com/seronet-project/SeRoNet-Tooling-ROS-Mixed-Port/tree/master/de.seronet_projekt.ros.componentGateway.generator#automatic-transformation-of-communication-objects)
...
Probably after the creation of the new Domain Models project and addition of the types and services files you will get an error because of missed dependencies.
...
Complementarity, and to make easier the relay of interfaces, we incorporated an extra help function that automatically relays ROS interfaces from an existing model. The previous function through its pop-up dialog guides the user to generate not only the rospoolinterface model but also a suggestion of a SeRoNet component with a ROS mixed Port. The resulted files will be generated under the folder *src-gen/SeRoNetComponent*. In case your ROS Model contains parameters, a new file with the format [componentParameters](http://www.servicerobotik-ulm.de/toolchain-manual/html/ch02s03s02.html#UsingToolchain_ComponentDevelopmentView_CompModeling_CompParameters) will also be autogenerated, which allow the transformation of ROS specifications to SeRoNet component models. The parameters bridge is only working for primitives types (Double, Integer, String, Boolean and Arrays of the previous mentioned types).

*![ROSSeRoNet_SeRoNetComponent](Screenshots/ROSSeRoNet_SeRoNetComponent.gif)*

These two files (alternatively three files, if the original ROS model had parameters) can be used to create a [new SeRoNet component](https://wiki.servicerobotik-ulm.de/tutorials:develop-your-first-component:start). First the perspective have to be changed to the "Component Supplier" one. This perspective will add the option to create a new Component  Project by giving a name and a localization. The previously generated  SeRoNetComponent files can simply be copied to the model folder of the  new project.
...
### ROS Mixed Port Walk Through tutorials
...
- From a ROS joy driver component relay a **ROS publisher** to the SeRoNet network (semi-automatic process: [ROSPublisher_examples](ROS-MixedPort-Examples/ROSPublisher_examples)
- Integrate a complete ROS system (model obtained automatically) with the the SeRoNet *SmartJoyStickServer* through a **Ros Subscriber**: [ROSSubscriber_examples](ROS-MixedPort-Examples/ROSSubscriber_examples)
- Call a **ROS Service** from SeRoNet to initialize a ROS native system component: [ROSServiceCall_examples](ROS-MixedPort-Examples/ROSServiceCall_examples)
- Connect a **ROS service Client** console call command with a SeRoNet component: [ROSClientCall_examples](ROS-MixedPort-Examples/ROSClientCall_examples)
